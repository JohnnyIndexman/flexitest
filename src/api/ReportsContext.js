...
export const ReportsProvider = ({ children }) => {
  ...
  const API_KEY = "$2a$10$wYLVusv1vQx5beP6t.0AjO0ijZWX6aaEJ7JgTQJx2wMJQhSPtaNNa";
  ...
  useEffect(() => {
    ...
    if (data && data.record && Array.isArray(data.record.record)) {
      ...
      console.error("Expected 'data.record' to be an array, but it is:", data.record);
      setReports([]); 
    }
      } catch (err) {
  ...
  //         'X-Master-Key': API_KEY,
  //       },
  //       body: JSON.stringify({ record: [...reports, newReport] }),
  //     });
  ...
  return (
    <ReportsContext.Provider
      value={{ reports, loading, error, createReport, updateReport, deleteReport, filteredReports, searchTerm, setSearchTerm}}
    >
      {children}
    </ReportsContext.Provider>
  );
